...
export default function VideoTable({
  ...
  const paginated = filtered.slice(
    (paginatedPage - 1) * perPage,
    paginatedPage * perPage,
  );
  ...
  const handleExport = () => {
    const header =
      "Title,Views,Likes,Comments,Engagement Rate,Published,Duration,URL\n";
    const rows = filtered
      .map(
        (v) =>
  ...
  return (
    <div>
      ...
      {/* Desktop Table */}
      <div className="hidden md:block overflow-x-auto rounded-xl border border-border max-w-[1200px]">
